...
class Lix {
  ...
  ///
  /// TIP: `readability` isn't passed to the constructor, but calculated.
  /// By adding the `late` keyword we tell the analyzer that it
  /// will be initialized later in the program.
  late int readability;

  /// TIP: Because the fields are all non-nullable,
  /// the constructor must initialize them all.
  /// We can either set a default value, or like here,
  /// mark the fields as `required`, meaning that
  /// a value must be passed to the constructor.
  /// `required` is a new keyword introduced as part of null safety
  ...
  /// yet we can return it as a non-nullable result without getting an error.
  ///
  /// This is possible due to "definite assignment": The Dart analyzer
  /// determines that a value has been definitely assigned before it's used.
  /// Try removing the assignment to `result` in
  /// one of the if/else code branches, and notice how
  /// an error then appears in the return statement.
  int _calculate() {
    int result;
  ...
  /// TIP: Notice how flow analysis knows that
  /// every branch returns a value for `readability` and so
  /// it's ok to have a non-nullable return type.
  /// Try removing the last else clause to see what happens.
  String describe() {
