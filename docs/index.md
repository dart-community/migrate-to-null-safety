...
description: >-
  An overview of Dart's null safety feature, including core principles,
  how to enable it across Dart versions, and links to migration resources.
---
...
For example, if a method expects an `int` but receives `null`,
the call results in a runtime error.
...
- To try some interactive examples,
  try out some of the null-safety oriented examples in the
...
Program-wide sound null safety lets Dart leverage these principles for
...
check out the [Dart 3 migration guide](https://dart.dev/resources/dart-3-migration).
...
Analyzing ....                         0.6s
  ...
  error - lib/pkg1.dart:1:1 - The language version must be >=2.12.0.
    Try removing the language version override and migrating the code. - illegal_language_version_override
```
...
1. Check for [null safe versions](migrate.md#check-dependency-status)
   of any packages you installed from pub.dev.
2. [Migrate](#migrating-existing-code) all of your source code to
   use sound null safety.

Dart 3 can be found in the stable channels for Dart and Flutter.
To download or update to Dart 3,
follow the instructions on [Get the Dart SDK](https://dart.dev/get-dart).
...
If the `dart pub get` step fails, check the
[status of your dependencies](migrate.md#check-dependency-status).

If the `analyze` step fails, update your code to
resolve the issues reported by the analyzer.
...
To enable sound null safety, set the [SDK constraint lower-bound][] to
an SDK version with a [language version][] of 2.12 or later.
...
[SDK constraint lower-bound]: https://dart.dev/to/sdk-constraint
...
>
> You can migrate without the tool, but it
> involves hand editing code.

Dart code written without null safety support can be
migrated to use and support null safety.
We recommend using the `dart migrate` tool,
which is included in Dart SDK versions 2.12 to 2.19.
...
check out the [migration guide](migrate.md).
...
* [Understanding null safety](concepts.md)
* [Migration guide for existing code](migrate.md)
* [Null safety FAQ](faq.md)
* [Unsound null safety](unsound.md)
* [Null safety sample code](../samples/calculate_lix)
