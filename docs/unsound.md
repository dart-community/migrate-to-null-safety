...
A Dart program might contain some libraries that
are [null safe](index.md) and some that aren't.
These **mixed-version programs**
rely on **unsound null safety**.

The ability to mix [language versions](https://dart.dev/resources/language/evolution#language-versioning)
...
> For details, see the [migration guide](migrate.md).
...
[migration tool](migrate.md#2-migrate).
To opt out files or directories, click the green checkbox.
In the following screenshot,
all files in the `bin` directory are opted out.

![Screenshot of file viewer in migration tool](img/migration-tool-incremental.png)
...
you can't un-migrate a file once it has been migrated.

[language version comment]: https://dart.dev/to/language-version-override
