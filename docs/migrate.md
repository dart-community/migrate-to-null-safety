---
title: Migrate to null safety
description: >-
  A step-by-step guide for migrating existing Dart code to null safety,
  using the dart migrate tool or migrating by hand.
---
...
> To learn more,
> see [Dart 3 and null safety](index.md#dart-3-and-null-safety).

This page describes how and when to migrate your code to [null safety](index.md).
Here are the basic steps for migrating each package that you own:

1. [**Wait**](#1-wait-to-migrate) for the packages
   that you depend on to migrate.
2. [**Migrate**](#2-migrate) your package's code,
   preferably using the interactive migration tool.
3. [**Statically analyze**](#3-analyze) your package's code.
4. [**Test**](#4-test) to make sure your changes work.
5. If the package is already on pub.dev,
   [**publish**](#5-publish) the null-safe version
...
watch
[How to migrate Dart packages to null safety](https://www.youtube.com/watch?v=eBr5tlumwlg)
on YouTube.
...
![Illustration of dependency migration order: A first, then B, then C](img/null-safety-migration-order.png)

Although you [_can_ migrate](unsound.md)
...
![Output of dart pub outdated](img/pub-outdated-output.png)
...
You can find contact details on the package page on [pub.dev](https://pub.dev).
...
[its type needs a `?` suffix](index.md#introduction-through-examples).
If a named parameter shouldn't be nullable,
mark it [`required`](concepts.md#required-named-parameters)
or give it a [default value](https://dart.dev/language/functions#default-parameters).

You have two options for migrating:

* [Use the migration tool](#using-the-migration-tool),
...
adding [hint markers](#improving-migration-results) to your Dart code.
...
![Screenshot of migration tool](img/migration-tool.png)
...
except for a 2.9 [language version comment](https://dart.dev/to/language-version-override).

For more information about incremental migration, see
[Unsound null safety](unsound.md).
...
The next step is to [statically analyze your code](#3-analyze).
If it's valid, then [test your code](#4-test).
Then, if you've published your code on pub.dev,
[publish a null-safe prerelease](#5-publish).
...
4. Migrate the code of each Dart file,
   using the analyzer to identify static errors.
   Eliminate static errors by adding `?`, `!`, `required`, and `late`,
   as needed.

See [Unsound null safety](unsound.md)
...
to perform [static analysis](https://dart.dev/tools/analysis) on your code:
...
* [Set the package version to indicate a breaking change.](#update-the-package-version)
* [Update the SDK constraints and package dependencies.](#check-your-pubspec)
* [Publish the package](https://dart.dev/tools/pub/publishing).
  If you don't consider this version to be a stable release,
  then [publish the package as a prerelease](https://dart.dev/tools/pub/publishing#publishing-prereleases).
...
If you haven't yet, update all [examples](https://dart.dev/tools/pub/package-layout#examples) and samples
